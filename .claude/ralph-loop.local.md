...
| Decision | Rationale |
|---|---|
| Full rewrite, not hybrid | Cleaner architecture; vanilla and Angular are incompatible in the DOM |
| Keep all form `id` attrs in Angular templates | Zero-change migration of modal e2e tests |
| `outputPath: "../wwwroot"` in `angular.json` | .NET serves Angular dist natively; no extra copy step |
| CSS custom props in global `styles.scss` | `:root` / `[data-theme]` must be global scope |
| `online` event as primary sync path | `BackgroundSync` API unsupported on Safari |
| Merge existing manifest values manually | Stable PWA `id` prevents double-install on existing users |
| `APP_INITIALIZER` for `authService.me()` | Non-blocking auth check on app start (same perf as vanilla) |

---

**Critical files to modify:**
- `Dockerfile` — add `node-build` stage before .NET stage
...
- `/client/` — Angular 17+ standalone project (Angular CLI scaffold)
- `/client/src/app/core/services/` — `AuthService`, `AutosService`, `FillupsService`, `ThemeService`, `ToastService`, `SyncQueueService`, `PushService`
- `/client/src/app/features/` — `login/`, `app-shell/`, `autos/`, `fillups/`
- `/client/src/app/shared/` — `toast/` component
- `/client/src/app/core/pipes/` — `FuelTypePipe`
- `/client/src/app/core/guards/` — `auth.guard.ts`
- `Endpoints/PushEndpoints.cs` — stub `POST /api/push/subscribe`
- New EF migration — `PushSubscriptions` table

---

**Work:**

1. **Task 21.1: Scaffold + Routing**
   - `ng new` into `/client`, configure output path `../wwwroot`, define routes: `/login`, `/app/log`, `/app/autos`
   - Add `app.MapFallbackToFile("index.html")` to `Program.cs` after `UseStaticFiles()` for SPA routing support

2. **Task 21.2: Auth Service + Login Component**
   - Build `AuthService` with `signal<User | null>` and computed `isAuthenticated` property
   - Create functional `authGuard` (CanActivateFn) protecting `/app/*` routes
   - Implement `LoginComponent` mirroring existing `index.html` card UI with same DOM `id` attributes for e2e compatibility

3. **Task 21.3: App Shell — Nav, Tabs, Auto Selector, Theme**
   - Build `AppShellComponent` with top nav, `<router-outlet>`, and tab routing via `routerLink`
   - Implement `AutosService` with `autos = signal<Auto[]>` and default-auto logic (most recently fueled)
   - Create `ThemeService` using `document.documentElement.setAttribute('data-theme', ...)` + `localStorage`
   - Port CSS custom properties from `app.html` to `styles.scss`; copy `assets/` to `client/src/assets/`

4. **Task 21.4: Autos Feature**
   - Build `AutosComponent`, `AutoCardComponent`, `AutoModalComponent` using ReactiveFormsModule
   - Implement `ToastService` + `ToastComponent` (replaces vanilla `#toast` div)
   - Wire CRUD operations through `AutosService`

5. **Task 21.5: Fillups Feature**
   - Build `FillupsComponent`, `FillupTableComponent`, `FillupModalComponent`
   - Implement `FillupsService` with `fillups = signal<FillupRow[]>` and `loading = signal<boolean>`
   - Keep GPS + Nominatim lookup inline in modal (browser-side, same as vanilla)
   - Create `FuelTypePipe` for enum rendering (0→Regular … 4→E85)
   - Wire `effect()` in FillupsComponent to react to `autosService.currentAutoId()` changes

6. **Task 21.6: PWA — Manifest, Service Worker, Caching**
   - Run `ng add @angular/pwa` and configure `ngsw-config.json` with appropriate caching strategy
   - Merge existing `manifest.json` values (stable `id`, `start_url`, `theme_color`, icons) into Angular's generated `manifest.webmanifest`
   - Implement `PwaService` with `SwUpdate` subscription → "New version available" toast
   - Verify offline shell loads and Lighthouse PWA audit score ≥ 90

7. **Task 21.7: Background Sync — Offline Fillup Queue**
   - Implement `SyncQueueService` using IndexedDB (`gasoholic-sync` db, `pending-fillups` store)
   - Update `FillupsService.save()` to enqueue fillups on network failure and show optimistic pending row
   - Register native `BackgroundSync` API (Chrome/Edge)
   - Implement `window.addEventListener('online', ...)` polling fallback (Safari/Firefox)
   - Use `BroadcastChannel` messaging to trigger table refresh after successful sync

8. **Task 21.8: Push Notifications**
   - Build `PushService` wrapping `SwPush`; load VAPID key from `environment.ts`
   - Add "Enable notifications" button in nav (hides after grant)
   - Implement backend `POST /api/push/subscribe` → `PushSubscriptions` table (create EF migration)
   - Show in-app toast for foreground push messages; navigate on notification click

9. **Task 21.9: Dockerfile Multi-Stage Build**
   - Add `node-build` stage (node:22-alpine): `npm ci` + `ng build --configuration=production`
   - Update .NET stage: `COPY --from=node-build /client/wwwroot/ ./wwwroot/`
   - Update `.dockerignore` to exclude `client/node_modules/`, `wwwroot/`, `client/dist/`
   - Verify Azure-deploy.yml requires no changes

10. **Task 21.10: Unit Tests (Jest)**
    - Replace Karma with Jest (`@angular-builders/jest`, `jest-preset-angular`)
    - Write specs for: `AuthService`, `AutosService`, `FillupsService`, `SyncQueueService`, `FuelTypePipe`, `LoginComponent`, `FillupModalComponent`
...
11. **Task 21.11: Playwright E2E Updates**
    ...
    - Verify full test suite passes against `dotnet run` + Angular dist
